---
doc_id: INTL-1
jurisdiction: international
legal_regime: treaty_patent
document_name: "Agreement on Trade-Related Aspects of Intellectual Property Rights (TRIPS)"
document_type: treaty
section_or_article: "Article 1 (Nature and Scope of Obligations)"
product_class_tags: ["all"]
date_enacted: "1994 (signed 15 April 1994, Marrakesh; effective 1 January 1995)"
last_verified_date: "2026-09-11"
source_url: "https://www.ippt.eu/legal-texts/trips/article-1"
status_note: "Verified against IP-PorTal's article-by-article TRIPS mirror (which itself states text in force verified against WTO.org) -- WTO's own trips_e legal-text page could not be fetched directly in this session (network restriction), so citing the mirror actually read rather than the WTO homepage. Recommend a direct WTO.org spot-check before demo if anyone has unrestricted access."
---

## Article 1 — Nature and Scope of Obligations

1. Members shall give effect to the provisions of this Agreement. Members may, but shall not be obliged to, implement in their law more extensive protection than is required by this Agreement, provided that such protection does not contravene the provisions of this Agreement. Members shall be free to determine the appropriate method of implementing the provisions of this Agreement within their own legal system and practice.

2. For the purposes of this Agreement, the term "intellectual property" refers to all categories of intellectual property that are the subject of Sections 1 through 7 of Part II.

3. Members shall accord the treatment provided for in this Agreement to the nationals of other Members. In respect of the relevant intellectual property right, the nationals of other Members shall be understood as those natural or legal persons that would meet the criteria for eligibility for protection provided for in the Paris Convention (1967), the Berne Convention (1971), the Rome Convention and the Treaty on Intellectual Property in Respect of Integrated Circuits, were all Members of the WTO members of those conventions. Any Member availing itself of the possibilities provided in paragraph 3 of Article 5 or paragraph 2 of Article 6 of the Rome Convention shall make a notification as foreseen in those provisions to the Council for Trade-Related Aspects of Intellectual Property Rights (the "Council for TRIPS").

## Why this matters for the India/International contrast (per build spec §18 demo script)

Article 1.1 is the "floor, not ceiling" provision: TRIPS sets minimum IP standards that WTO members (including India) must implement, but members remain free to legislate more extensive protection or exclusions domestically as long as they don't contravene TRIPS. This is the treaty-level basis for why India's Patents Act §3(p)/§3(d) traditional-knowledge and incremental-innovation bars (see `legal-corpus/india/patents-act-1970/`) are TRIPS-compliant even though they are stricter/more specific than TRIPS itself — India is adding domestic detail on top of the TRIPS floor, not falling below it. When the demo switches from India to International mode for the same product, this article is the reason the answer is framed as "the international baseline that India's law then implements and adds to," not a separate, unrelated legal system.
