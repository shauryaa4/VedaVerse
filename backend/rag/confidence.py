...
SUPPORT_SCORE_ABSTAIN_BELOW = (
    0.50  # combined with low classification confidence -> abstain
)
SUPPORT_SCORE_HEDGE_BELOW = (
    0.75  # below this alone -> hedge, even with high classification confidence
)
...
def compute_confidence_decision(
...
) -> tuple[ConfidenceDecision, ConfidenceLevel, str]:
    """
    ...
    if citation_support_score < SUPPORT_SCORE_HEDGE_BELOW:
        reasons.append(
            f"only {citation_support_score:.0%} of claims were citation-supported"
        )
    if status_notes:
        reasons.append("the matched law includes a status caveat (see notes)")

    if reasons:
        # Within "hedge", still distinguish MEDIUM from LOW for the badge:
        # weak citation support drags the badge to LOW even if classification
        # confidence alone was fine -- citation support is the harder signal.
        hedge_level: ConfidenceLevel = (
            "low" if citation_support_score < SUPPORT_SCORE_ABSTAIN_BELOW else "medium"
        )
        return "hedge", hedge_level, "; ".join(reasons) + "."

    return (
        "answer",
        "high",
        "Classification confidence and citation support were both sufficient.",
    )
...
def apply_confidence_decision(
    ...
    """
    rag_response.confidence = level
    rag_response.confidence_reason = reason
    ...
    if decision == "abstain":
        ...
        if not rag_response.answer_text.endswith(HEDGE_DISCLAIMER):
            rag_response.answer_text = (
                rag_response.answer_text.rstrip() + HEDGE_DISCLAIMER
            )
